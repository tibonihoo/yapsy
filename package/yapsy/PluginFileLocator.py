...
class IPluginFileAnalyzer(object):
    ...
    def isValidPlugin(self, filename):
        ...
        raise NotImplementedError(
            "'isValidPlugin' must be reimplemented by %s" %
            self)
    ...
    def getInfosDictFromPlugin(self, dirpath, filename):
        ...
        *filename* is the name (ie the basename) of the plugin file.
        ...
        """
        raise NotImplementedError(
            "'getInfosDictFromPlugin' must be reimplemented by %s" %
            self)
...
class PluginFileAnalyzerWithInfoFile(IPluginFileAnalyzer):
    ...
    def getPluginNameAndModuleFromStream(
            self,
            infoFileObject,
        ...
        if not config_parser.has_option(
                "Core",
                "Name") or not config_parser.has_option(
                "Core",
                "Module"):
            log.debug(
    ...
    def _extractBasicPluginInfo(self, directory, filename):
        ...
        if infos and config_parser and config_parser.has_section(
                "Documentation"):
    ...
    def getInfosDictFromPlugin(self, dirpath, filename):
        ...
        if not infos or infos.get("name", None) is None:
            raise ValueError(
                "Missing *name* of the plugin in extracted infos.")
        if not infos or infos.get("path", None) is None:
            raise ValueError(
                "Missing *path* of the plugin in extracted infos.")
        return infos, config_parser
...
class PluginFileLocator(IPluginLocator):
    ...
    def removeAnalyzers(self, name):
        ...
        if not foundAndRemoved:
            log.debug(
                "'%s' is not a known strategy name: can't remove it." %
                name)
    ...
    def _getInfoForPluginFromAnalyzer(self, analyzer, dirpath, filename):
        ...
        plugin_info_cls = self._plugin_info_cls_map.get(
            analyzer.name,
            self._default_plugin_info_cls)
        plugin_info = plugin_info_cls(
            plugin_info_dict["name"],
            plugin_info_dict["path"])
...
#						print candidate_infofile
                        ...
                        if plugin_info is None:
                            log.warning(
                                ...
                                (candidate_infofile, analyzer.name))
                            # we consider this was the good strategy to use
                            # for: it failed -> not a plugin -> don't try
                            # another strategy
                            break
...
#					print candidate_filepath
                        _candidates.append(
                            (candidate_infofile,
                             candidate_filepath,
                             plugin_info))
                        # finally the candidate_infofile must not be discovered
                        # again
    ...
    def getPluginNameAndModuleFromStream(
        ...
        for analyzer in self._analyzers:
            if analyzer.name == "info_ext":
                return analyzer.getPluginNameAndModuleFromStream(
                    infoFileObject)
        else:
            raise RuntimeError(
