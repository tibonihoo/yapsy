...
class PluginManager(object):
    ...
    def setPluginLocator(self, plugin_locator, dir_list=None, picls=None):
        ...
        if isinstance(plugin_locator, IPluginLocator):
            ...
            raise TypeError(
                "Unexpected format for plugin_locator ('%s' is not an instance of IPluginLocator)" %
                plugin_locator)
    ...
    def _gatherCorePluginInfo(self, directory, plugin_info_filename):
        ...
        """
        return self.getPluginLocator().gatherCorePluginInfo(
            directory,
    ...
    def _getPluginNameAndModuleFromStream(
            self,
            infoFileObject,
            candidate_infofile="<buffered info>"):
        """
        DEPRECATED(>1.9): please use a specific plugin
        locator if you need such information.
        ...
        and decorators.
        """
        return self.getPluginLocator().getPluginNameAndModuleFromStream(
            infoFileObject,
            candidate_infofile)
...
# 		print "%s.loadPlugins" % self.__class__
        ...
        for candidate_infofile, candidate_filepath, plugin_info in self._candidates:
            ...
            for element in (
                getattr(
                    candidate_module,
                    name) for name in dir(candidate_module)):
                ...
                for category_name in self.categories_interfaces:
                    ...
                    # XXX here we also get RunTimeError if running in a Flask
                    # environment
                    except Exception:
                        continue
                    if is_correct_subclass and element is not self.categories_interfaces[
                        ...
                        if candidate_infofile not in self._category_file_mapping[
                                current_category]:
                            # we found a new plugin: initialise it and search
                            # for the next one
