...
class AutoInstallPluginManager(PluginManagerDecorator):
    ...
    def install(self, directory, plugin_info_filename):
        ...
        if not (
            os.path.exists(
                plugin_info.path) or os.path.exists(
                plugin_info.path +
                ".py")):
            log.warning(
                "Could not find the plugin's implementation for %s." %
                plugin_info.name)
            return False
    ...
    def installFromZIP(self, plugin_ZIP_filename):
        ...
        if not os.path.isfile(plugin_ZIP_filename):
            log.warning(
                "Could not find the plugin's zip file at '%s'." %
                plugin_ZIP_filename)
            return False
        try:
            ...
            if first_bad_file:
                raise Exception(
                    "Corrupted ZIP with first bad file '%s'" %
                    first_bad_file)
        except Exception as e:
            log.warning(
                "Invalid zip file '%s' (error: %s)." %
                (plugin_ZIP_filename, e))
            return False
        zipContent = candidateZipFile.namelist()
        log.info(
            "Investigating the content of a zip file containing: '%s'" %
            zipContent)
        log.info(
        ...
        isValid = False
        log.info(
            "Looking for the zipped plugin's info file among '%s'" %
            infoFileCandidates)
        for infoFileName in infoFileCandidates:
            infoFile = candidateZipFile.read(infoFileName)
            log.info(
                "Assuming the zipped plugin info file to be '%s'" %
                infoFileName)
