...
class ConfigurablePluginManager(PluginManagerDecorator):
    ...
    def __getCategoryPluginsListFromConfig(self, plugin_list_str):
        ...
        """
        return plugin_list_str.strip(" ").split(
            "%s" %
    ...
    def __addPluginToConfig(self, category_name, plugin_name):
        ...
        if not self.config_parser.has_option(
            ...
            past_list_str = self.config_parser.get(
                self.CONFIG_SECTION_NAME,
                option_name)
            ...
            if plugin_name not in past_list:
                ...
                new_list_str = self.__getCategoryPluginsConfigFromList(
                    past_list)
                self.config_parser.set(
                    self.CONFIG_SECTION_NAME,
                    option_name,
    ...
    def hasOptionFromPlugin(self,
        ...
        return self.config_parser.has_section(
            section_name) and self.config_parser.has_option(section_name, option_name)
    ...
    def __decoratePluginObject(
        ...
        """
        plugin_object.setConfigOption = lambda x, y: self.registerOptionFromPlugin(
            category_name, plugin_name, x, y)
        plugin_object.setConfigOption.__doc__ = self.registerOptionFromPlugin.__doc__
        plugin_object.getConfigOption = lambda x: self.readOptionFromPlugin(
            category_name,
            plugin_name,
            x)
        plugin_object.getConfigOption.__doc__ = self.readOptionFromPlugin.__doc__
        plugin_object.hasConfigOption = lambda x: self.hasOptionFromPlugin(
            category_name,
            plugin_name,
            x)
        plugin_object.hasConfigOption.__doc__ = self.hasOptionFromPlugin.__doc__

    def activatePluginByName(
            self,
            plugin_name,
        ...
        if pta is None:
            return None
        self.__decoratePluginObject(
            category_name,
            plugin_name,
            pta.plugin_object)
        # activate the plugin
        plugin_object = self._component.activatePluginByName(
            plugin_name,
            category_name)
    ...
    def deactivatePluginByName(
        ...
        """
        # activate the plugin
        plugin_object = self._component.deactivatePluginByName(
            plugin_name,
            category_name)
        ...
        if self.config_parser.has_section(self.CONFIG_SECTION_NAME):
            ...
            for category_name in list(self._component.category_mapping.keys()):
                ...
                if self.config_parser.has_option(self.CONFIG_SECTION_NAME,
                                                 option_name):
                    plugin_list_str = self.config_parser.get(
                        self.CONFIG_SECTION_NAME,
                        option_name)
                    plugin_list = self.__getCategoryPluginsListFromConfig(
                        plugin_list_str)
