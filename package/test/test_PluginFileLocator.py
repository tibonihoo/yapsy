    ...
    def test_deprecated_method_dont_raise_notimplemetederror(self):
        ...
        self.assertEqual(
            (None,
             None,
             None),
            dpl.getPluginNameAndModuleFromStream(None))
...
class PluginFileAnalyzerWithInfoFileTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.plugin_directory = os.path.join(
            ...
            "plugins")
        self.yapsy_plugin_path = os.path.join(
            self.plugin_directory,
            "simpleplugin.yapsy-plugin")
        self.version_plugin_path = os.path.join(
            self.plugin_directory,
            "versioned11.version-plugin")
        self.yapsy_filter_plugin_path = os.path.join(
            self.plugin_directory,
    ...
    def test_getInfosDictFromPlugin(self):
        ...
        info_dict, cf_parser = analyzer.getInfosDictFromPlugin(
            self.plugin_directory, os.path.basename(
                self.yapsy_plugin_path))
        self.assertEqual(info_dict,
                         {'website': 'http://mathbench.sourceforge.net',
                          ...
                          'version': '0.1',
                          'path': '%s/SimplePlugin' % self.plugin_directory,
                          'name': 'Simple Plugin',
                          'copyright': '2014'})
    ...
    def test_isValid_WithMultiExtensions(self):
        analyzer = PluginFileAnalyzerWithInfoFile(
            "mouf", ("yapsy-plugin", "yapsy-filter-plugin"))
        self.assertTrue(analyzer.isValidPlugin(self.yapsy_plugin_path))
        self.assertFalse(analyzer.isValidPlugin(self.version_plugin_path))
...
class PluginFileAnalyzerMathingRegexTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.yapsy_plugin_path = os.path.join(
            self.plugin_directory,
            "SimplePlugin.py")
        self.version_plugin_10_path = os.path.join(
            self.plugin_directory,
            "VersionedPlugin10.py")
        self.version_plugin_12_path = os.path.join(
            self.plugin_directory,
    ...
    def test_getInfosDictFromPlugin(self):
        ...
        info_dict, cf_parser = analyzer.getInfosDictFromPlugin(
            self.plugin_directory, os.path.basename(
                self.version_plugin_10_path))
        self.assertEqual(
            info_dict, {
                'path': self.version_plugin_10_path, 'name': 'VersionedPlugin10'})
        self.assertTrue(isinstance(cf_parser, ConfigParser))
...
class PluginFileLocatorTest(unittest.TestCase):
    ...
    def test_locatePlugins_recursively_when_plugin_is_a_symlinked_directory(
            self):
        temp_dir = tempfile.mkdtemp()
        try:
            temp_sub_dir = os.path.join(temp_dir, "plugins")
    ...
    def test_gatherCorePluginInfo(self):
        pl = PluginFileLocator()
        plugin_info, cf_parser = pl.gatherCorePluginInfo(
            self.plugin_directory, "simpleplugin.yapsy-plugin")
        self.assertTrue(plugin_info.name, "Simple Plugin")
        self.assertTrue(isinstance(cf_parser, ConfigParser))
        plugin_info, cf_parser = pl.gatherCorePluginInfo(
            self.plugin_directory, "notaplugin.atall")
...
class PluginManagerSetUpTest(unittest.TestCase):
    ...
    def test_updatePluginPlaces(self):
        ...
        self.assertEqual(
            set(["bla/bli", "mif/maf"]), set(pm.getPluginLocator().plugins_places))
    ...
    def test_setPluginInfoClass_with_strategies(self):
        ...
        self.assertEqual(
            {"mouf": SpecificPluginInfo, "hop": SpecificPluginInfo}, loc.icls)
