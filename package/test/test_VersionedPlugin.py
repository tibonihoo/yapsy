...
class VersionedTestsCase(unittest.TestCase):
    ...
    def plugin_loading_check(self):
        ...
        if self.plugin_info is None:
            ...
            self.assertEqual(
                len(self.versionedPluginManager.getCategories()), 1)
            ...
            self.assertEqual(
                len(self.versionedPluginManager.getPluginsOfCategory(sole_category)), 1)
            # older versions of the plugin should be found in the attic
            self.assertEqual(
                len(self.versionedPluginManager.getPluginsOfCategoryFromAttic(sole_category)), 4)
            plugins = self.versionedPluginManager.getPluginsOfCategory(
                sole_category)
            self.plugin_info = None
    ...
    def testLoaded(self):
        ...
        sole_category = self.versionedPluginManager.getCategories()[0]
        self.assertEqual(
            len(self.versionedPluginManager.getLatestPluginsOfCategory(sole_category)), 1)
        self.plugin_info = self.versionedPluginManager.getLatestPluginsOfCategory(
            sole_category)[0]
