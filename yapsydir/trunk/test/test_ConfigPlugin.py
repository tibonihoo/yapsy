...
from yapsy.ConfigurablePluginManager import ConfigurablePluginManager, ConfigurablePluginManagerSingleton
...
class ConfigSingletonTestsCase(unittest.TestCase):
	...
	def setUp(self):
		...
		# create the plugin manager
		pluginManager = ConfigurablePluginManagerSingleton.get()
	...
	def testActivationAndDeactivation(self):
		...
		"""
		self.plugin_activate()
		ConfigurablePluginManagerSingleton.get().deactivatePluginByName(self.plugin_info.category,
																		self.plugin_info.name)
		self.assert_(not self.plugin_info.plugin_object.is_activated)
	...
	def plugin_activate(self):
		...
		if not self.plugin_info.plugin_object.is_activated:
			ConfigurablePluginManagerSingleton.get().activatePluginByName(self.plugin_info.category,
																		  self.plugin_info.name)
		self.assert_(self.plugin_info.plugin_object.is_activated)
